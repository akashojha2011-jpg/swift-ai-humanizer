...
import { Sliders, Wand2, Copy, Check, Sparkles, FileText } from "lucide-react";
import { humanizeText, HumanizeTone, convertMarkdownToHTML } from "@/lib/humanizerEngine";

export function ToneCustomizerStudio() {
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
        {tones.map((t) => (
          <button
            ...
            className={`p-4 rounded-2xl text-left border transition-all flex flex-col justify-between space-y-2 cursor-pointer ${
              selectedTone === t.id
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-3xl shadow-float">
        ...
        <div className="space-y-4 flex flex-col justify-between">
          ...
          <button
            ...
            className="w-full py-3.5 rounded-full font-heading font-bold text-xs text-white bg-brand-600 hover:bg-brand-700 disabled:opacity-40 shadow-md flex items-center justify-center gap-2 transition-all cursor-pointer"
          >
        ...
        <div className="space-y-4 flex flex-col justify-between">
          ...
          {output && (
            <button
              onClick={handleCopy}
              className="w-full py-3.5 rounded-full font-heading font-bold text-xs text-brand-900 dark:text-emerald-200 bg-brand-50 dark:bg-emerald-950 border border-brand-200 dark:border-emerald-800 hover:bg-brand-100 flex items-center justify-center gap-2 transition-all cursor-pointer"
            >
