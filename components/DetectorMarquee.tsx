      ...
      <div className="relative w-full overflow-hidden pt-2 pb-1">
        ...
        <div className="flex w-max items-center gap-10 sm:gap-14 animate-marquee hover:[animation-play-state:paused]">
          {marqueeList.map((detector, idx) => (
            <div
              key={idx}
              className="flex items-center gap-2.5 shrink-0 opacity-85 hover:opacity-100 transition-opacity cursor-default hover:scale-105"
            >
              {detector.icon}
              <span className={`font-heading font-bold text-sm sm:text-base ${detector.color}`}>
                {detector.name}
              </span>
            </div>
          ))}
